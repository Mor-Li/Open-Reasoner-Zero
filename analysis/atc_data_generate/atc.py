...
try:
    ...
    # We're running in the OpenCompass environment
    IN_OPENCOMPASS = True
except ImportError:
    # Running standalone - create minimal implementations needed
    print("Running in standalone mode without OpenCompass")
    IN_OPENCOMPASS = False
...
class NeedleBenchATCDataset(BaseDataset):
    ...
    def load(
        ...
        language: str,
        repeats: int,
        # 这个参数无法通过mmengine传入f过来因为被block为lazy了
...
if __name__ == '__main__':
    ...
    def generate_examples(question_type, language, needle_counts, repeats, seed, names_data, is_training=True):
        """生成指定配置的示例"""
        examples = []
        all_names = names_data[language].split(',')
        base_seed = seed
        ...
        # 选择适当的模板和术语
        if language == 'Chinese':
            rel_terms = relationship_terms_zh_CN
            rel_templates = relationship_templates_zh_CN
            rel_map = relationship_generation_map_zh
        else:  # English
            rel_terms = relationship_terms_en
            rel_templates = relationship_templates_en
            rel_map = relationship_generation_map_en
        
        for num_needles in tqdm(needle_counts, desc=f"Processing {question_type.name.lower()} {language.lower()}"):
            for i in range(repeats):
                # 生成唯一的种子
                example_seed = base_seed + (num_needles * 1000) + i
                ...
                for j in range(len(names) - 1):
                    ...
                    relation_term = random.choice(rel_terms)
                    relation = template.format(A=names[j],
                                             B=names[j + 1],
                                             relationship=relation_term)
                    story += f'{relation}*'
                    
                    # 获取这种关系的代数差异
                    gen_diff = rel_map.get(relation_term, 1)
                    total_generations += gen_diff
                    
                    # 记录关系信息
                    relationships.append((names[j], names[j + 1], relation_term, gen_diff))
                
                # 分割并打乱故事
                family_story_fragments = story.split('*')
                family_story_fragments = [f for f in family_story_fragments if f]
                random.shuffle(family_story_fragments)
                shuffled_story = ''.join(family_story_fragments)
                ...
                if question_type == QuestionType.ELDEST_ANCESTOR:
                    ...
                    if language == 'Chinese':
                        ...
                        prompt = relationship_distance_prompt_en.format(
                            shuffled_story=shuffled_story, person_a=person_a, person_b=person_b)
                    answer = str(total_generations)  # 关系距离是总代际数
                
                # 根据is_training参数决定格式
                if is_training:
                    example = [
                        {"from": "human", "value": prompt},
                        {"from": "assistant", "ground_truth": {
                            "value": answer,
                            "names": names,  # 添加所有人名，方便reward function使用
                            "needle_count": num_needles,  # 添加针数信息
                            "total_generations": total_generations  # 添加总代数信息
                        }}
                    ]
                else:
                    # 评估数据使用简单格式
    ...
    parser = argparse.ArgumentParser(description='Generate ATC training and evaluation data')
    parser.add_argument('--output_dir', type=str, default='data', 
                        help='Output directory for generated data')
    parser.add_argument('--name_file', type=str, default='analysis/atc_data_generate/names.json',
                        help='File containing names for generating relationships')
    parser.add_argument('--language', type=str, default='both', choices=['English', 'Chinese', 'both'],
                        help='Language for generated data')
    parser.add_argument('--question_types', type=str, default='all',
                        help='Question types to generate (comma-separated)')
    parser.add_argument('--needle_counts', type=str, default='2,4,8,16,32,64,128,256,512',
                        help='Comma-separated list of needle counts')
    parser.add_argument('--train_repeats', type=int, default=10,
                        help='Number of training examples per configuration')
    parser.add_argument('--eval_repeats', type=int, default=5,
                        help='Number of evaluation examples per configuration')
    ...
    # 确定使用的语言
    languages = ['English', 'Chinese'] if args.language == 'both' else [args.language]
    
    # 解析问题类型
    if args.question_types.lower() == 'all':
        question_types = [
            QuestionType.ELDEST_ANCESTOR,
            QuestionType.NTH_ANCESTOR, 
            QuestionType.NTH_DESCENDANT,
        ...
        question_types = []
        type_names = args.question_types.split(',')
        for name in type_names:
            name = name.strip().upper()
            if name == 'ELDEST_ANCESTOR':
                question_types.append(QuestionType.ELDEST_ANCESTOR)
            elif name == 'NTH_ANCESTOR':
                question_types.append(QuestionType.NTH_ANCESTOR)
            elif name == 'NTH_DESCENDANT':
                question_types.append(QuestionType.NTH_DESCENDANT)
            elif name == 'RELATIONSHIP_DISTANCE':
                question_types.append(QuestionType.RELATIONSHIP_DISTANCE)
            else:
                print(f"Warning: Unknown question type '{name}', skipping.")
    
    # 解析针数列表
    needle_counts = [int(x.strip()) for x in args.needle_counts.split(',')]
    
    # 加载名称文件
    try:
        with open(args.name_file, 'r', encoding='utf-8') as file:
            names_data = json.load(file)
    except Exception as e:
        print(f"Error loading names file: {e}")
        sys.exit(1)
    
    print(f"Generating ATC data with the following settings:")
    print(f"- Languages: {languages}")
    print(f"- Question types: {[qt.name for qt in question_types]}")
    print(f"- Needle counts: {needle_counts}")
    print(f"- Training repeats per configuration: {args.train_repeats}")
    print(f"- Evaluation repeats per configuration: {args.eval_repeats}")
    print(f"- Output directory: {args.output_dir}")
    
    # 确保输出目录存在
    train_dir = os.path.join(args.output_dir)
    eval_dir = os.path.join(args.output_dir, "eval_data")
    ...
    for question_type in question_types:
        for is_training in [True, False]:
            for lang in languages:
                ...
                # 生成明确不同的种子范围
                # 训练数据和评估数据使用完全不同的种子空间
                type_seed = args.seed + (question_type.value * 1000000)
                if lang == 'Chinese':
                    type_seed += 10000000  # 为中文数据添加偏移
                if not is_training:
                    type_seed += 100000000  # 为评估数据添加大偏移量
                
                # 生成数据...
                print(f"Generating {data_type} data for {question_type.name.lower()} in {lang}...")
                examples = generate_examples(
                    ...
                    is_training=is_training  # 直接传入当前是否为训练数据
                )
                ...
                print(f"Generated {len(examples)} {data_type} examples for {question_type.name.lower()} in {lang} and saved to {output_path}")
