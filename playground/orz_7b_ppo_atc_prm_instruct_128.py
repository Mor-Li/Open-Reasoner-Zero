...
DEBUG_MODE=True python -m playground.orz_7b_ppo_atc_prm

Multi-node Training:

on master node, first run `ray start --head`
then on other nodes, run `ray start --address='<master-node-ip>:<master-node-port>'`
then on master node, run `python -m playground.orz_7b_ppo_atc_prm`
...
class PPOExpConfig(BasePPOExpConfig):
    ...
    prompt_data: ListConfig = ListConfig(
        [
            "data/atc_data_128/ea_zh.json",  # 祖先追溯测试-中文
            "data/atc_data_128/ea_en.json",  # 祖先追溯测试-英文
            "data/atc_data_128/na_zh.json",  # 第n代祖先-中文
            "data/atc_data_128/na_en.json",  # 第n代祖先-英文
            "data/atc_data_128/nd_zh.json",  # 第n代后代-中文
            "data/atc_data_128/nd_en.json",  # 第n代后代-英文
            "data/atc_data_128/rd_zh.json",  # 亲属距离-中文
            "data/atc_data_128/rd_en.json",  # 亲属距离-英文
        ]
    )
    eval_prompt_data: ListConfig = ListConfig(
        [
            "data/atc_data_128/eval_data/ea_zh.json",
            "data/atc_data_128/eval_data/ea_en.json",
            "data/atc_data_128/eval_data/na_zh.json",
            "data/atc_data_128/eval_data/na_en.json",
            "data/atc_data_128/eval_data/nd_zh.json",
            "data/atc_data_128/eval_data/nd_en.json",
            "data/atc_data_128/eval_data/rd_zh.json",
            "data/atc_data_128/eval_data/rd_en.json",
        ]
    )
...
class CustomRewardTrainer(RayPPOTrainer):
    ...
    async def custom_reward_fn(
        ...
        @ray.remote(num_cpus=1)
        def check_names_in_reasoning(response, names):
            # 检查推理过程中出现了多少个人名
            # 首先尝试提取思考过程，查找<think>...</think>标签
            think_pattern = re.compile(r"<think>(.*?)</think>", re.DOTALL)
            think_match = re.search(think_pattern, response)
            
            reasoning_text = think_match.group(1) if think_match else response
            
            # 计算有多少个名字出现在推理过程中
            names_found = 0
            for name in names:
                if name in reasoning_text:
                    names_found += 1
            
            return names_found, len(names)

        rep_tasks = []  # 存储重复度和反思模式计算的远程任务
        name_check_tasks = []  # 存储检查名字出现的远程任务
        
        for idx, (output, extra) in enumerate(zip(outputs, extras)):
            ...
            # 检查推理过程中出现的人名
            names = []
            if "ground_truth" in extra and "names" in extra["ground_truth"]:
                names = extra["ground_truth"]["names"]
            name_check_tasks.append(check_names_in_reasoning.remote(response, names))
            
        rep_task_results = ray.get(rep_tasks)  # 获取所有远程任务的结果
        name_check_results = ray.get(name_check_tasks)  # 获取名字检查任务的结果
        ...
        for idx in range(len(outputs)):
            ...
            # 获取名字检查结果
            names_found, total_names = name_check_results[idx]
            name_ratio = names_found / total_names if total_names > 0 else 0.0
            
            # 将重复度和反思模式得分添加到输出信息中
            output["repeat_score"] = rep_score
            output["reflection_pattern_score"] = reflection_pattern_score
            output["names_found"] = names_found
            output["total_names"] = total_names
            output["name_ratio"] = name_ratio
            
            # 计算新的奖励分数
            # 基于答案正确性和推理过程的完整性计算奖励
            if stop_reason == "stop":
                # 计算奖励：完全基于推理过程中提到的人名比例
                # 如果答案正确，奖励为人名覆盖率
                # 如果答案错误，奖励为人名覆盖率的30%
                if iscorrect:
                    score = name_ratio  # 满分1.0，完全由推理中提到的人名比例决定
                else:
                    score = 0.3 * name_ratio  # 答案错误，但仍然给予部分推理质量奖励
            else:
                # 如果模型没有正常停止，得分很低且仍基于人名比例
                avg_non_stop_count += 1
                score = 0
                
            # 如果推理中没有提到任何人名，无论最终答案如何，分数都为0
            if names_found == 0:
                score = 0.0
        ...
        # 提取正确和错误响应的token数量
        correct_tokens_arr = np.array([]) if np.all(scores_arr == 0) else np.array(num_tokens_arr[scores_arr >= 0.5])
        incorrect_tokens_arr = np.array([]) if np.all(scores_arr >= 0.5) else np.array(num_tokens_arr[scores_arr < 0.5])
        ...
        log_dict = {
            ...
            "avg_names_ratio": sum(output["name_ratio"] for output in outputs) / len(outputs),  # 平均人名覆盖率
    ...
    async def eval(self):
        ...
        for file_name in all_file_names:
            # 使用缩写名称或保持原样（如果映射中没有）
            abbrev_name = dataset_name_abbrev.get(file_name, file_name)
            dump_file_name += f"_{abbrev_name}{log_dict[f'{file_name}/accuracy']:.4f}"
